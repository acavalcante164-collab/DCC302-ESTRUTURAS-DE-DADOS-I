...
Lista* merge_sorted_lists(Lista *L1, Lista *L2){
    ...
    Lista* rabu =cabeca; //rabu aponta para o ultimo no do l3

    Lista* p1 = L1; //PONTEIRO PARA PERCORRER L2
    Lista* p2 = L2; //PONTEIRO PARA PERCORRER L2

    while (p1 != NULL && p2 != NULL){
        if(p1->info <= p2->info){
            //copia o valor de p1
            rabu->prox = cria(p1->info);
            p1 = p1->prox;
        } else {
            //copia o valor de p2
            rabu->prox = cria(p2->info);
            p2 = p2->prox;            
        }
        //AVANCA O RABU DA NOVA LISTA
        rabu = rabu->prox;
    }
    // Se L1 ainda tem elementos
    while (p1 != NULL){
        rabu->prox = cria(p1->info);
        p1 = p1->prox;
        rabu = rabu->prox;
    }
    // Se L2 ainda tem elementos
    while (p2 != NULL){
        rabu->prox = cria(p2->info);
        p2 = p2->prox;
        rabu = rabu->prox;
    ...
    Lista* L3 = cabeca->prox;
    free(cabeca);
    return L3;
}
int main (void) {
//CRIA A PRIMEIR LISTA
Lista* l1 = inicializa();
...
//MESCLA AS DUAS
Lista* l3_iter = merge_sorted_lists(l1, l2);
printf("\nLista Mesclada L3:\n");
imprime(l3_iter);
