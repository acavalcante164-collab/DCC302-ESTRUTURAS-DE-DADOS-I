...
/*
REMOVE ELEMENTOS DUPLICADOS DE UMA LISTA ORDENADA
MANTENDO APENAS A PRIMEIRA OCORRENCIA
*/
void remove_duplicates(Lista * L){
    //SE A LISTA TÁ VAZIA OU APENAS UM ELEMENTO
    //NAO TEM DUPLICADAS PARA REMOVER
    if (L == NULL || L->prox == NULL){
        return;
    }
    Lista* atual = L; //PONTEIRO PARA PERCORRER A LISTA

    //percorre ate atual ou ate atual -> prox seja NULL
    while (atual != NULL && atual->prox != NULL){
        //compara o no atual com o proximo
        if(atual->info == atual->prox->info){
            //duplicadas encontradas
            Lista* no_para_remover = atual->prox;

            atual->prox = no_para_remover->prox;
...
l = insere_ordenado(l, 20); // Duplicata de 20
l = insere_ordenado(l, 40); // Duplicata de 40
l = insere_ordenado(l, 10); // Duplicata de 10
l = insere_ordenado(l, 30); // Duplicata de 30
l = insere_ordenado(l, 30); // Outra duplicata de 30

printf("LISTA ORIGINAL:");
imprime(l);

//chamando para remover duplicadas
remove_duplicates(l);
