...
/*
funcao get_kth_element: retorna o valor do k-esimo elemento
assumindo que k =0 e oprimeiro elemento, k=1 eh o segundo..
retorna -1 se o indice 'k' for invalido (negativo ou maior que a lista)

*/
int get_kth_element(Lista* l, int k){
    ...
    Lista* p = l;
    int i;
    //percorre a lista, avancando o poteiro p k vezes
    //para se  p== NULL i == K
    ...
    if(p == NULL){
        //se for null acabou antes de encontrar o resultado
        return -1;
    } else{
        return p ->info;
        //ele esta apontando para o k esimo no
    }
}
int main (void) {
...
printf("Elemento 0 (K = 0): %d\n", get_kth_element(l,0));
printf("Elemento 1 (k = 1): %d\n", get_kth_element(l,1));
printf("Elemento 3 (k = 3): %d\n", get_kth_element(l,3));

// Testes inválidos
printf("Elemento 4 (k=4): %d\n", get_kth_element(l, 4));   // Esperado: -1 (fora dos limites)
printf("Elemento 10 (k=10): %d\n", get_kth_element(l, 10)); // Esperado: -1 (fora dos limites)
printf("Elemento -1 (k=-1): %d\n", get_kth_element(l, -1)); // Esperado: -1 (negativo)
