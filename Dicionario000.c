...
// NOVA FUNÇÃO: Imprime o arquivo original com números de linha para conferência
void imprimir_arquivo_base(char *nome_arquivo) {
    ...
    int linha = 1;
    while (fgets(buffer, 1024, f) != NULL) {
        // Remove \n visual para o print ficar bonito
        buffer[strcspn(buffer, "\n")] = 0; 
        printf("Linha %02d: \"%s\"\n", linha++, buffer);
    }
    printf("==========================================\n");
...
int main() {
    ...
    do {
        ...
        switch (op) {
            ...
            case 4: 
                // Chama a visualização do arquivo ANTES do índice
